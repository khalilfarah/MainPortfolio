...
export function Contact({ismode}){
    const notify = () => toast("thank you , successfuly send to khalil ");
    const form = useRef();
    const sendEmail = (e) => {
      e.preventDefault();
    ...
    return(
        <section id="Contact" className={`mx-auto  items-center pt-8  px-1 justify-center   ${ismode ?"bg-gray-100 ":"bg-gradient-to-r from-gray-700 via-gray-800 to-gray-900"}`}>
            ...
            <div className="mx-auto w-1/4 pb-4 ">
              <button onClick={notify} title={btnClasses?"please enter info above":""}  type="submit"  disabled={btnIsDisabled} className={`text-center w-full mx-auto justify-center  px-6 py-2 font-bold ${btnClasses ?"bg-gray-600 text-gray-300 cursor-not-allowed":"bg-cyan-500 text-white cursor-pointer"} `}>Send</button>
            </div>
            </form>
